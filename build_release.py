...
def main():
    ...
    payload = output / 'JuicyCensor-2.0.3-Windows-x64'
    if payload.exists():
    ...
    zip_folder(payload, output/'JuicyCensor-2.0.3-Windows-x64-Portable.zip', payload.name)
    source = output / 'JuicyCensor-2.0.3-Source'
    source.mkdir()
    ...
    zip_folder(source, output/'JuicyCensor-2.0.3-Source.zip', source.name)
